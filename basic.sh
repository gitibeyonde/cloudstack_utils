...
dns_ext=8.8.8.8
dns_int=192.168.100.1
nmask=255.255.255.0
hpvr=Xenserver

pod_gw=192.168.100.1
pod_start=192.168.100.65
pod_end=192.168.100.95

vlan_gw=172.16.16.1
vlan_start=172.16.16.55
vlan_end=172.16.16.95
...
pod_id=`$cli create pod name=NinjaPod zoneid=$zone_id gateway=$pod_gw netmask=$nmask startip=$pod_start endip=$pod_end | grep ^id\ = | awk '{print $3}'`
echo "Created pod"

$cli create vlaniprange podid=$pod_id networkid=$net_id gateway=$vlan_gw netmask=$nmask startip=$vlan_start endip=$vlan_end forvirtualnetwork=false
...
$cli create storagepool zoneid=$zone_id podid=$pod_id clusterid=$cluster_id name=NinjaPrimary scope=cluster hypervisor=$hpvr url=$prm_storage
...
$cli update user id=$user_id password=password
