...
PSTESTS="test_deploy_vgpu_enabled_vm test_deploy_vm_root_resize test_deploy_vm_with_userdata test_hosts test_internal_lb test_loadbalance test_scale_vm test_snapshots test_vm_snapshots"
...
for suite in $PSTESTS; do 
	nosetests --with-xunit --xunit-file=$TMP/prov_smoke/$suite.xml --with-marvin --marvin-config=/root/cloudstack_utils/bootcamp_advanced.cfg $CLOUDDIR/test/integration/smoke/$suite.py -s -a tags=advanced,required_hardware=true --zone=Bootcamp --hypervisor=xenserver 
done
