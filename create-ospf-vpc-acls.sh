...
vpc2_vm001_name=Robin-VPC-VM-001
vpc2_vm001_display_name='"Robin VPC VM 001"'
vpc2_vm001_ip_addr=200.100.5.101

vpc2_vm002_name=Robin-VPC-VM-002
vpc2_vm002_display_name='"Robin VPC VM 002"'
vpc2_vm002_ip_addr=200.100.6.102
...
vpc2_tier2_name=robin-tier-2
vpc2_tier2_display_name='"Robin Tier 2"'
vpc2_tier2_gateway=200.100.6.1
vpc2_tier2_netmask=255.255.255.0
...
vpc_offering_id=`$cli list vpcofferings name='"Default Dynamically Routed VPC offering"' | grep ^id\ = | awk '{print $3}'`
echo "VPC Offering ID = "$vpc_offering_id

vpc_t1_offering_id=`$cli list networkofferings name=DefaultIsolatedDynamicRoutedNetworkOfferingForVpcNetworks | grep ^id\ = | awk '{print $3}'`
echo "VPC Tier 1 Offering ID = "$vpc_t1_offering_id

vpc_t2_offering_id=`$cli list networkofferings name=DefaultIsolatedDynamicRoutedNetworkOfferingForVpcNetworks | grep ^id\ = | awk '{print $3}'`
...
echo " "
 ...
 #create second VPC
 echo "About to create second VPC, check if the first one is up and fine"
 echo "Press Enter to Continue" ; read
...
echo "Created Tier 2 ACL List - ID = "$tier2_acllist_id
echo "Tier 2 ACL List Created - Please wait whilst we now add the ACL Rules"
tier2_acl1_id=`$cli create networkacl protocol=tcp aclid=$tier2_acllist_id cidrlist=172.16.5.0/24 startport=3306 endport=3306 traffictype=ingress action=allow | grep ^id\ = | awk '{print $3}'`
echo "Rule 1 Created"
...
# ************************ Create Networks ************************
echo "Please wait whilst we create the Tier 1 Network"
vpc_tier1_network_id=`$cli create network zoneid=$zone_id vpcid=$vpc_id domainid=$domain_id account=$account networkofferingid=$vpc_t1_offering_id name=$vpc2_tier1_name displaytext=$vpc2_tier1_display_name gateway=$vpc2_tier1_gateway netmask=$vpc2_tier1_netmask aclid=$tier1_acllist_id | grep ^id\ = | awk '{print $3}'`
echo "Created Tier 1 Network - ID = "$vpc_tier1_network_id
echo " "

echo "Please wait whilst we create the Tier 2 Network"
vpc_tier2_network_id=`$cli create network zoneid=$zone_id vpcid=$vpc_id domainid=$domain_id account=$account networkofferingid=$vpc_t2_offering_id name=$vpc2_tier2_name displaytext=$vpc2_tier2_display_name gateway=$vpc2_tier2_gateway netmask=$vpc2_tier2_netmask aclid=$tier2_acllist_id | grep ^id\ = | awk '{print $3}'`
echo "Created Tier 2 Network - ID = "$vpc_tier2_network_id
echo " "

# ************************ Create VPC2 VM 001 ************************
echo "$vpc2_vm001_name is deploying - please wait - first VM can take a long time to deploy as Template may require copying"
vm001_jobid=`$cli deploy virtualmachine serviceofferingid=$serviceoffering_id templateid=$template_id zoneid=$zone_id account=$account domainid=$domain_id networkids=$vpc_tier1_network_id ipaddress=$vpc2_vm001_ip_addr name=$vpc2_vm001_name displayname=$vpc2_vm001_display_name | grep ^jobid\ = | awk '{print $3}'`
echo "Deploy VM Job ID ="$vm001_jobid
